...
class RedisStatsProvider(object):
    def __init__(self):
        ...
        startup_nodes = [{"host":stats_server["server"], "port":stats_server["port"]}]
    ...
    def save_keys_Info(self, server,rediskey,timestamp, expires, persists,expired,evicted
        ...
        data = str(score)+','+str(commands)+','+str(expires)+','+str(persists)+','+str(expired)+','+str(evicted)+','+str(hit_rate)+','+str(peak)+','+str(used)
    ...
    def get_keys_info(self, server, from_date, to_date):
        ...
        for row in rows:
            ...
            if(index % rate == 0):
                array = row.split(',')
                timestamp = datetime.fromtimestamp(int(array[0]))
